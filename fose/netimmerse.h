...
template <class T>
class __declspec(empty_bases) NiPointer : public NiMemObject
{
public:
	...
	__forceinline NiPointer(T* apObject) : m_pObject(apObject)
	{
		if (m_pObject) m_pObject->IncRefCount();
	}

	__forceinline NiPointer(const NiPointer& arOther) : m_pObject(arOther.m_pObject)
	{
		if (m_pObject) m_pObject->IncRefCount();
	}


	__forceinline ~NiPointer()
	{
	...
	__forceinline bool operator==(T* apObject) const
	{
		return (m_pObject == apObject);
	}

	__forceinline bool operator==(const NiPointer& ptr) const
	{
...
struct alignas(16) AlignedVector4
{
	...
	__forceinline AlignedVector4(const AlignedVector4& rhs)
	{
		*this = rhs;
	}

	__forceinline explicit AlignedVector4(const __m128 rhs)
	{
		SetPS(rhs);
	}
	...
	__forceinline void operator=(const AlignedVector4& rhs)
	{
		SetPS(rhs.PS());
	}
	...
	__forceinline __m128 PS() const
	{
		return _mm_load_ps(&x);
	}
};

using hkVector4 = AlignedVector4;
...
public:
	...
	friend NiPoint3 operator*(float fScalar, const NiPoint3& pt)
	{
	...
	{
		[[msvc::flatten]] return std::sqrt(x * x + y * y + z * z);
	}
	...
	NiPoint3 UnitCross(const NiPoint3& pt) const
	{
		...
		if (fLength > 1e-06f) return cross / fLength;
...
public:
	...
	NiMatrix3()
	{
		memset(m_pEntry, 0, sizeof(m_pEntry));
	}
...
public:
	...
	void DecRefCount()
	{
		if (!InterlockedDecrement(&m_uiRefCount)) DeleteThis();
	}
};
...
public:
	...
	virtual void LinkObject(void* apStream);
	virtual void RegisterStreamables(void* apStream);
	...
	virtual void PostLinkObject(void* apStream);
...
public:
	explicit NiUpdateData(float afTime = 0.f, bool abUpdateControllers = false, bool abParallelUpdate = false)
		: fTime(afTime), bUpdateControllers(abUpdateControllers), bParallelUpdate(abParallelUpdate), bFoundParticles(false),
		  bFoundMorphController(false), bSceneGraphChange(false)
	{
	}
...
static_assert(sizeof(NiUpdateData) == 0xC);
...
public:
	...
	{
		visible ? (m_flags &= ~kNiFlag_Culled) : (m_flags |= kNiFlag_Culled);
	};

	bool IsVisible()
	{
		return !(m_flags & kNiFlag_Culled);
	}
	...
	bool IsSelectiveUpdate()
	{
		return m_flags & kNiFlag_SelectiveUpdate;
	};
...
public:
	...
	NiTArray<NiAVObjectPtr> m_children; // 9C
