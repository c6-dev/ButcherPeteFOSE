...
{
	if (code > ScriptParsing::ScriptStatementCode::Max) return nullptr;
...
bool ScriptParsing::ScriptContainsCommand(Script* script, CommandInfo* info, CommandInfo* eventBlock)
{
	...
	if (analyzer.error) return false;
	if (analyzer.CallsCommand(info, eventBlock)) return true;
	return false;
}
...
bool ScriptParsing::PluginDecompileScript(Script* script, SInt32 lineNumber, char* buffer, UInt32 bufferSize)
{
	...
	if (lineNumber != -1)
	{
		if (lineNumber >= analyzer.lines.size()) return false;
...
ScriptParsing::ScriptIterator::ScriptIterator(Script* script, UInt16 opcode, UInt16 length, UInt16 refIdx, UInt8* data) :
	opcode(opcode), length(length), refIdx(refIdx), script(script), curData(data), startData(data)
{
}
...
{
	return curData > script->data + script->info.dataLength;
}
...
ScriptParsing::ScriptCommandCall::ScriptCommandCall(const ScriptIterator& contextParam) : ScriptLine(contextParam,
		cmdTableInterface->GetByOpcode(contextParam.opcode)),
	commandCallToken(std::make_unique<CommandCallToken>(contextParam))
{
	if (error) return;
	if (context.length)
	{
		if (!commandCallToken->ParseCommandArgs(context, context.length)) error = true;
	}
}
...
ScriptParsing::ScriptStatement::ScriptStatement(const ScriptIterator& contextParam) : ScriptLine(
	contextParam, GetScriptStatement(static_cast<ScriptStatementCode>(contextParam.opcode)))
{
}
...
ScriptParsing::BeginStatement::BeginStatement(const ScriptIterator& contextParam) : ScriptStatement(contextParam)
{
	...
	this->commandCallToken = std::make_unique<CommandCallToken>(eventBlockCmd, eventBlockCmd ? eventBlockCmd->opcode : -1,
	                                                            nullptr, context.script);
	if (context.length > 6) // if more than eventOpcode and endJumpLength
	{
		...
		if (!commandCallToken->ParseCommandArgs(iter, iter.length - 6)) error = true;
	}
}
...
{
	return ScriptLine::ToString() + " " + (this->commandCallToken ? this->commandCallToken->ToString() : "");
}
...
std::string ScriptParsing::OperatorToken::ToString(std::vector<std::string>& stack)
{
	...
	{
		if (stack.empty()) return "<invalid number of operands on stack>";
		const auto topOp = stack.back();
		stack.pop_back();
		return '-' + topOp;
	}
	if (stack.size() < 2) return "<invalid number of operands on stack>";
...
std::string ScriptParsing::ScriptVariableToken::GetBackupName()
{
	...
	std::string prefix = "i";
	if (varInfo->idx == Script::eVarType_Float)
	{
		if (varInfo->IsReferenceType(this->script)) prefix = "r";
		else prefix = "f";
	}
	return prefix + "Var" + std::to_string(varInfo->idx);
}

ScriptParsing::ScriptVariableToken::ScriptVariableToken(Script* script, ExpressionCode varType, VariableInfo* info,
                                                        TESForm* ref) : OperandToken(varType), script(script), ref(ref),
                                                                        varInfo(info)
{
	if (!this->varInfo || !this->script) error = true;
}
...
ScriptParsing::StringLiteralToken::StringLiteralToken(const std::string_view& stringLiteral) : stringLiteral(stringLiteral)
{
}
...
ScriptParsing::GlobalVariableToken::GlobalVariableToken(Script::RefVariable* refVariableParam) :
	RefToken(nullptr, refVariableParam),
...
ScriptParsing::CommandCallToken::CommandCallToken(CommandInfo* cmdInfo, UInt32 opcode, Script::RefVariable* callingRef,
                                                  Script* script) : opcode(opcode), cmdInfo(cmdInfo)
{
	if (!cmdInfo) error = true;
	if (callingRef) callingReference = std::make_unique<RefToken>(script, callingRef);
}

ScriptParsing::CommandCallToken::CommandCallToken(const ScriptIterator& iter) : CommandCallToken(
	cmdTableInterface->GetByOpcode(iter.opcode), iter.opcode, iter.GetCallingReference(), iter.script)
{
	opcode = iter.opcode;
}
...
{
	if (!cmdInfo) return FormatString("UNK_OPCODE_%04X", this->opcode);
	std::string refStr;
	if (callingReference) refStr = callingReference->ToString() + '.';
	...
	return refStr + TokenListToString(this->args, [&](auto& token, UInt32 i)
	{
		...
		if (auto* numToken = dynamic_cast<NumericConstantToken*>(&arg))
		{
			...
			if (auto str = StringForNumericParam(typeID, value); !str.empty()) return str;
		}
		return arg.ToString();
	});
}
...
ScriptOperator* ScriptParsing::Expression::ReadOperator()
{
	...
	const auto isNonOperatorChar = [](unsigned char c)
	{
		return c <= 0x20 || isspace(c) || !ispunct(c);
	};
	if (isNonOperatorChar(*context.curData)) return nullptr;
...
bool ScriptParsing::CommandCallToken::ReadFormToken(ScriptIterator& context)
{
	...
	{
		if (!ReadVariableToken(context)) return false;
	}
	return true;
}
...
bool ScriptParsing::CommandCallToken::ParseGameArgs(ScriptIterator& context, UInt32 numArgs)
{
	...
	for (auto i = 0u; i < numArgs; ++i)
	{
		...
		case kExtractParam_StringLiteral:
			{
				...
				if (context.ReadStringLiteral(stringLit)) args.push_back(std::make_unique<StringLiteralToken>(stringLit));
				break;
			}
		...
		case kExtractParam_Form:
			if (!ReadFormToken(context)) return false;
			break;
		}
		if (args.back()->error) return false;
	}
	return true;
}
...
constexpr UInt32 g_messageBoxParseCmds[] = {0x51C800, 0x51DB50, 0x51DB80, 0x51DBB0};
...
bool ScriptParsing::CommandCallToken::ParseCommandArgs(ScriptIterator context, UInt32 dataLen)
{
	...
	if (context.curData < context.startData + dataLen &&
		Contains(g_messageBoxParseCmds, reinterpret_cast<UInt32>(cmdInfo->parse)))
	{
		// message box args?
		const auto numArgs = context.Read16();
		...
		for (auto i = 0u; i < numArgs; ++i)
		{
			if (!ReadNumericToken(context)) return false;
		}
	}
	return true;
}
...
bool ScriptParsing::Expression::ReadExpression()
{
	...
	while (context.curData < endData)
	{
		...
		if (curChar <= 0x20 || isspace(curChar)) continue;
		switch (curChar)
		{
		case 's':
		case 'l':
		case 'f':
			{
				...
				this->stack.push_back(std::make_unique<ScriptVariableToken>(script, static_cast<ExpressionCode>(curChar),
				                                                            script->GetVariableInfo(varIdx),
				                                                            refVar ? refVar->form : nullptr));
				refVar = nullptr;
				continue;
			}
		case 'G':
		...
		case '"':
			{
				...
				if (context.ReadStringLiteral(strLit)) this->stack.push_back(std::make_unique<StringLiteralToken>(strLit));
				break;
			}
		case 'X':
			{
				const auto opcode = context.Read16();
				auto token = std::make_unique<CommandCallToken>(cmdTableInterface->GetByOpcode(opcode), opcode, refVar,
				                                                context.script);
				const auto dataLen = context.Read16();
				if (!token->error && dataLen && !token->ParseCommandArgs(context, dataLen)) return false;
...
std::string ScriptParsing::Expression::ToString()
{
	...
	for (auto iter = this->stack.begin(); iter != this->stack.end(); ++iter)
	{
		...
		if (auto* operatorToken = dynamic_cast<OperatorToken*>(token.get()))
		{
			if (!operatorStack.empty() && !operands.empty())
			{
				...
				if (operatorToken->scriptOperator->precedence > lastOp->precedence)
				{
					...
					if (composites.contains(rhOperand)) rhOperand = '(' + rhOperand + ')';
					if (operands.size() > 1)
					{
						auto& lhOperand = operands.at(operands.size() - 2);
						if (composites.contains(lhOperand)) lhOperand = '(' + lhOperand + ')';
					}
				}
			}
		...
		else if (auto* operandToken = dynamic_cast<OperandToken*>(token.get()))
		{
			...
			if (iter + 1 != stack.end() && (callToken = dynamic_cast<CommandCallToken*>(token.get())))
			{
				...
				if (std::ranges::any_of(paramInfo, [](auto& p)
				{
					return p.isOptional;
				}))
				{
	...
	if (operands.size() != 1)
	{
		return "; failed to decompile expression, bad expression stack:" + TokenListToString(
			operands, [&](const std::string& s, unsigned i)
			{
...
ScriptParsing::SetToStatement::SetToStatement(const ScriptIterator& contextParam) : ScriptStatement(contextParam)
{
	...
	if (!expression.ReadExpression()) error = true;
}
...
ScriptParsing::ConditionalStatement::ConditionalStatement(const ScriptIterator& contextParam) : ScriptStatement(contextParam),
	expression(context)
{
	if (error) return;
...
{
	if (token.cmdInfo == cmd) return true;
	return false;
}
...
bool ScriptParsing::ScriptAnalyzer::CallsCommand(CommandInfo* cmd, CommandInfo* eventBlockInfo = nullptr)
{
	...
	return std::ranges::any_of(this->lines, [&](std::unique_ptr<ScriptLine>& line)
	{
		...
		if (eventBlockInfo && (beginStatement = dynamic_cast<BeginStatement*>(line.get()))) lastEventBlock = beginStatement->
			eventBlockCmd;
		if (eventBlockInfo && lastEventBlock && lastEventBlock != eventBlockInfo) return false;
		if (line->cmdInfo == cmd) return true;
		ScriptCommandCall* cmdCall;
		CommandCallToken* argsToken;
		if ((cmdCall = dynamic_cast<ScriptCommandCall*>(line.get())) && (argsToken = cmdCall->commandCallToken.get()) &&
			DoesTokenCallCmd(*argsToken, cmd)) return true;
		if ((beginStatement = dynamic_cast<BeginStatement*>(line.get())) && beginStatement->eventBlockCmd == cmd) return true;
		if (auto* expressionStatement = dynamic_cast<ExpressionStatement*>(line.get()))
		{
			...
			for (auto& token : expression.stack)
			{
				...
				if (!(cmdCallToken = dynamic_cast<CommandCallToken*>(token.get()))) continue;
				if (DoesTokenCallCmd(*cmdCallToken, cmd)) return true;
			}
		}
		return false;
	});
...
void ScriptParsing::ScriptAnalyzer::Parse()
{
	while (!this->iter.End())
	{
		...
		if (lines.back()->error) error = true;
		++this->iter;
	}
}
...
{
	return FormatString("%d.%d.%d", FOSE_VERSION_INTEGER, FOSE_VERSION_INTEGER_MINOR, FOSE_VERSION_INTEGER_BETA);
}
...
std::string ScriptParsing::ScriptAnalyzer::DecompileScript()
{
	...
	auto numTabs = 0;
	const auto nestAddOpcodes = {
		static_cast<UInt32>(ScriptStatementCode::If),
		static_cast<UInt32>(ScriptStatementCode::Begin)
	};
	const auto nestMinOpcodes = {
		static_cast<UInt32>(ScriptStatementCode::EndIf),
		static_cast<UInt32>(ScriptStatementCode::End)
	};
	const auto nestNeutralOpcodes = {
		static_cast<UInt32>(ScriptStatementCode::Else),
		static_cast<UInt32>(ScriptStatementCode::ElseIf)
	};
	UInt32 lastOpcode = 0;
	for (auto& it : this->lines)
	{
		...
		if (isAdd && !scriptText.ends_with("\n\n") && !Contains(nestAddOpcodes, lastOpcode) && !Contains(
			nestNeutralOpcodes, lastOpcode)) scriptText += '\n';
		if (isMin) --numTabs;
		const auto isNeutral = Contains(nestNeutralOpcodes, opcode);
		if (isNeutral) --numTabs;
		if (numTabs < 0) numTabs = 0;

		if (!scriptText.ends_with("\n\n") && Contains(nestMinOpcodes, lastOpcode) && !Contains(nestMinOpcodes, opcode) && !
			Contains(nestNeutralOpcodes, opcode)) scriptText += '\n';

		auto nextLine = it->ToString();
		if (it->error) nextLine += " ; there was an error decompiling this line\n";
	...
	// Build header string
		...
		auto addVar = [&](VariableInfo* var)
		{
			...
			if (var->IsReferenceType(script)) scriptHeader += "Ref ";
			else if (var->type == Script::VariableType::eVarType_Float) scriptHeader += "Float ";
			else scriptHeader += "Int ";
...
size_t __stdcall ScriptParsing::DecompileToBuffer(Script* pScript, FILE* pStream, char* pBuffer)
{
	if (ScriptAnalyzer analyzer(pScript); !analyzer.error)
	{
		...
		{
			compiledSrc += '\n';
			if (pStream) fwrite(compiledSrc.c_str(), compiledSrc.length(), 1, pStream);
			if (pBuffer) memcpy(pBuffer, compiledSrc.c_str(), compiledSrc.length() + 1);
